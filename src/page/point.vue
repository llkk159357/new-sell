<template>
    <div class="point">
    <headtitle title="我的积分"></headtitle>
    <div class="wrapper">
      <div class="inner">
        <div class="current">
            <span>当前积分</span>
            <div class="intro">
             <span class="el-icon-question"></span>
             <span v-on:click="topointintro">积分说明</span>
         </div>
           
        </div>
        <div>
        <span class="num">0</span><span>分</span>
        </div>
        <div class="bottombtn">
            <button v-on:click="pop">积分兑换商品</button>
        </div>
        </div>  
    </div>
    <div class="about">
        最近30天积分兑换
    </div>
    <div class="detail">
        <img src="data:image/png;base64," alt="">
        <div>
         <div class="record">最近30天无积分记录</div>   
         <div>快去下单赚取大量积分吧</div>
        </div>
    </div>
       <transition name="slide">
      <div class="popout" v-if="poped">
        <div class="el-icon-warning"></div>
        <div class="earn">快去下单赚取大量积分吧</div>
        <div class="sure" v-on:click="concell">确认</div>
      </div>
       </transition>
       <transition name="fade">
       <router-view></router-view>
       </transition>
    </div>
</template>
<script>
import headtitle from '../components/headtitle'
export default {
    data(){
      return {
          poped:false
      }
    },
    components:{
        headtitle
    },
    methods:{
        pop:function(){
            this.poped=!this.poped
        },
        concell:function(){
            this.poped=!this.poped
        },
        topointintro:function(){
        this.$router.push('/point/pointintro')
        }
    }
}
</script>
<style scoped>
  .point{
      font-size: 0.7rem;
      color:grey;
      position: relative;
      height:100%;
  }
   .current{
       display: flex;
       justify-content: space-between;

   }
   .wrapper{
       background:#409EFF;
       padding:0.3rem 0.5rem;
   }
   .inner{
       background:white;
       padding:0.4rem 0.5rem;
       
   }
   button{
      background-color: #fe6d47;
       width:100%;
       height:2.2rem;
       border:none;
       border-radius: 5px;
       color:white;
       font-size: 0.9rem;
   }
   .current{
       font-size: 0.7rem;
       margin-bottom: 0.5rem;
   }
   .num{
       font-size: 1.8rem;
       margin-right:0.3rem;
   }
   .intro{
       color:#409EFF; 
   }
   .bottombtn{
       margin-top:0.8rem;
   }
   .about{
       margin-top:0.5rem;
       padding:0 0.5rem;
   }
   .detail{
       text-align: center;
       margin-top:2rem;
   }
   .record{
       font-size: 0.8rem;
       margin-bottom: 0.3rem;
   }
   .detail img{
       width:10rem;
       margin-bottom: 0.6rem;
   }
   .popout{
     text-align: center;
    position: absolute;
    width:75%;
    height:6rem;
    border-radius: 8px;
    background:white;
    top:50%;
    left:12.5%;
   }
   .sure{
       background:#4cd964;
       height:2rem;
       margin-top:1rem;
       line-height: 2rem;
       font-size: 0.8rem;
       color:white;
       border-bottom-left-radius: 8px;
       border-bottom-right-radius: 8px;
   }
   .el-icon-warning{
       font-size: 3rem;
       color:orange;
   }
   .earn{
    margin-top:1.2rem;
   }
   .slide-enter-active{
       animation: scale 0.4s 
   }

   @keyframes scale{
       0%{transform: scale(1)}
       50%{transform: scale(0.5)}
       100%{transform: scale(1)}
   }
   .fade-enter,.fade-leave-active{
    transform: translate(4rem);
  opacity: 0;
   }
   .fade-enter-active,.fade-leave-active{
       transition:all 0.5s ;
       
   }


</style>


